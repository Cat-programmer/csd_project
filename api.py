...
------WebKitFormBoundaryb6ZrB1LvoGELHGVQ--\r
    ...
    def load(self,id,path):
        ...
        if newjpgtxt:
            ...
            image_result = open(f'{path}', 'wb')
            image_result.write(image_64_decode)
            return path # Возвращаем True, если все прошло успешно


def generate(prompt, path='', style='', max_calls = 240):
    client = rudalleClient()
    status, id = client.ask(prompt,style)
    if status != True:return False
    x = client.check(id)[0]
    calls = 0
    while x != True & calls < max_calls:
        sleep(0.5)
        x = client.check(id)[0]
        calls+=1
    if calls < max_calls:
...
async def async_generate(prompt, path='', style=''):
    return await asyncio.to_thread(generate, prompt, path, style)